...
# IMPORTANT: the QEMU monitor command "boot order" sends key 'd' to GRUB so it picks
# the verbose menuentry, otherwise 'quiet splash' suppresses everything we want to grep.
...
( tail -n +1 -f "$SERIAL_LOG" 2>/dev/null & TAIL_PID=$!
  while kill -0 $QEMU_PID 2>/dev/null; do
    if grep -qE 'aurora login:|Welcome to AuroraOS|Started LightDM|reached target Graphical|aurora@aurora|live-config: applying' "$SERIAL_LOG" 2>/dev/null; then
      sleep 8        # let a bit more userspace come up before we kill
...
check    "GRUB menu reached"          'AuroraOS  -  Live Session|Booting .AuroraOS'
check    "Kernel handed off to userspace" 'aurora login:|systemd\[1\]|Welcome to|live-config|live-boot'
check    "Login prompt available"     'aurora login:'
...
check_no "No squashfs read errors"    'SQUASHFS error'
check_no "No initramfs failure"       '\(initramfs\) $|BusyBox.*built-in shell.*\(initramfs\)'
